...
def single_linkage(points, clusters, p, q):
	...
	'''
	distances = []
	p_cluster = clusters[p]
	q_cluster = clusters[q]

	for x_point in p_cluster._items:
		for y_point in q_cluster._items:
			distance = euclidean_distance(points[x_point], points[y_point])
...
class MyAgglomerativeClustering():
	...
	def find_clusters_to_merge(self):
		...
		None
		-------
		Returns:self._clusters
		p, q: int, int
			the id of two clusters that should be merged
		'''
		min_distance = np.inf
		p, q = 0, 0
		for a in range(len(self._clusters) - 1):
			if self._clusters[a].used is True:
				for b in range(a + 1, len(self._clusters)):
					if self._clusters[b].used is True:
						distance = self._proximity_matrix[a][b]
	...
	def merge_cluster(self, p, q):
		...
		p_cluster = self._clusters[p]
		q_cluster = self._clusters[q]

		p_items = p_cluster._items
		q_items = q_cluster._items

		new_cluster = MyCluster()
		new_cluster.extend(p_items)
		new_cluster.extend(q_items)
	...
	def update_proximity(self, new_cluster):
		...
		Returns:
			None
		'''
		for idx in range(len(self._clusters)):
			if self._clusters[idx].used is True and idx is not new_cluster:
				distance = self._linkage_func(self._items, self._clusters, new_cluster, idx)
				self._proximity_matrix[idx][new_cluster] = distance
